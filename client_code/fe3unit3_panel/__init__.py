...
class fe3unit3_panel(fe3unit3_panelTemplate):
  ...
  def unit_drop_change(self, **event_args):
    ...
    if self.parent.combat.duels[2].unit.name in ("Astram", "Abel", "Est", "Ogma", "Catria", "Gordin", "Samson", "Caeda", "Sheena", "Julian", "Sirius", "Cecil", "Palla", "Bantu", "Phina", "Merric", "Marth", "Midia", "Minerva", "Jubelo", "Yuliya", "Ryan", "Linde", "Luke", "Lena", "Roderick"):
    ...
    if self.parent.combat.duels[2].unit.name not in ("Jagen", "Bord", "Cord", "Barst", "Vyland", "Sedgar", "Wolf", "Hardin", "Caesar", "Radd", "Dolph", "Macellan", "Tomas", "Boah", "Lorenz"):
  ...
  def support1_change(self, **event_args):
    ...
    self.parent.combat.duels[2].unit.setsupportbonus(self.support1.text, self.support1.checked)
    self.parent.combat.duels[2].unitdisplay()
    self.hit.text = self.parent.combat.duels[2].unit.hit
    self.crit.text = self.parent.combat.duels[2].unit.crit
  ...
  def support2_change(self, **event_args):
    """This method is called when this checkbox is checked or unchecked"""
    self.parent.combat.duels[2].unit.setsupportbonus(self.support2.text, self.support2.checked)
    self.parent.combat.duels[2].unitdisplay()
    self.hit.text = self.parent.combat.duels[2].unit.hit
    self.crit.text = self.parent.combat.duels[2].unit.crit
