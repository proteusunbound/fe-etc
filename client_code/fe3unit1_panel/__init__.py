"""Unit 1 Panel"""
from ._anvil_designer import fe3unit1_panelTemplate
from anvil import *
...
class fe3unit1_panel(fe3unit1_panelTemplate):
  """Unit Template"""
  ...
  def unit_drop_change(self, **event_args):
    ...
    if self.parent.combat.duels[0].unit.name in ("Astram", "Abel", "Est", "Ogma", "Catria", "Gordin", "Samson", "Caeda", "Sheena", "Julian", "Sirius", "Cecil", "Palla", "Bantu", "Phina", "Merric", "Marth", "Midia", "Minerva", "Jubelo", "Yuliya", "Ryan", "Linde", "Luke", "Lena", "Roderick"):
    ...
    if self.parent.combat.duels[0].unit.name not in ("Jagen", "Bord", "Cord", "Barst", "Vyland", "Sedgar", "Wolf", "Hardin", "Caesar", "Radd", "Dolph", "Macellan", "Tomas", "Boah", "Lorenz"):
  ...
  def book2box_change(self, **event_args):
    """This method is called when this checkbox is checked or unchecked"""
    self.parent.combat.duels[0].unit.booktwo(self.book2box.checked)
    self.hp.text = self.parent.combat.duels[0].unit.maxhp
    self.strength.text = self.parent.combat.duels[0].unit.strength
    self.skill.text = self.parent.combat.duels[0].unit.skill
    self.speed.text = self.parent.combat.duels[0].unit.speed
