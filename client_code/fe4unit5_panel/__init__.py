...
class fe4unit5_panel(fe4unit5_panelTemplate):
  ...
  def unit_drop_change(self, **event_args):
    ...
    if self.parent.combat.duels[4].unit.charclass in (
      "Junior Lord",
      "Princess",
      "Prince",
      "Cavalier",
  ...
  def hpboost_pressed_enter(self, **event_args):
    """This method is called when the user presses Enter in this text box"""
    self.parent.combat.duels[4].unit.boosthp(self.hpboost.text)
    self.hp.text = self.parent.combat.duels[4].unit.maxhp
    self.startinghp.text = self.parent.combat.duels[4].unit.maxhp

  def strboost_pressed_enter(self, **event_args):
    """This method is called when the user presses Enter in this text box"""
    self.parent.combat.duels[4].unit.boost_strength(self.strboost.text)
    self.strength.text = self.parent.combat.duels[4].unit.strength

  def magboost_pressed_enter(self, **event_args):
    """This method is called when the user presses Enter in this text box"""
    self.parent.combat.duels[4].unit.boostmagic(self.magboost.text)
    self.magic.text = self.parent.combat.duels[4].unit.magic

  def sklboost_pressed_enter(self, **event_args):
    """This method is called when the user presses Enter in this text box"""
    self.parent.combat.duels[4].unit.boost_skill(self.sklboost.text)
    self.skill.text = self.parent.combat.duels[4].unit.skill
    self.parent.combat.duels[4].unitdisplay()
    self.hit.text = self.parent.combat.duels[4].unit.hit
    self.crit.text = self.parent.combat.duels[4].unit.crit
  ...
  def spdboost_pressed_enter(self, **event_args):
    """This method is called when the user presses Enter in this text box"""
    self.parent.combat.duels[4].unit.boost_speed(self.spdboost.text)
    self.speed.text = self.parent.combat.duels[4].unit.speed
    self.parent.combat.duels[4].unitdisplay()
    self.attackspeed.text = self.parent.combat.duels[4].unit.AS
  ...
  def lckboost_pressed_enter(self, **event_args):
    """This method is called when the user presses Enter in this text box"""
    self.parent.combat.duels[4].unit.boostluck(self.lckboost.text)
    self.luck.text = self.parent.combat.duels[4].unit.luck

  def defboost_pressed_enter(self, **event_args):
    """This method is called when the user presses Enter in this text box"""
    self.parent.combat.duels[4].unit.boostdefense(self.defboost.text)
    self.defense.text = self.parent.combat.duels[4].unit.defense

  def resboost_pressed_enter(self, **event_args):
    """This method is called when the user presses Enter in this text box"""
    self.parent.combat.duels[4].unit.boostresistance(self.resboost.text)
    self.resistance.text = self.parent.combat.duels[4].unit.resistance

  def killcount_pressed_enter(self, **event_args):
    """This method is called when the user presses Enter in this text box"""
    self.parent.combat.duels[4].unitweapon.setkillcount(self.killcount.text)
    self.parent.combat.duels[4].adjustunitskills()
    self.parent.combat.duels[4].unitdisplay()
    self.crit.text = self.parent.combat.duels[4].unit.crit
  ...
  def refreshbox_change(self, **event_args):
    """This method is called when this checkbox is checked or unchecked"""
    self.parent.combat.duels[4].refresh = self.refreshbox.checked

  def father_drop_change(self, **event_args):
    """This method is called when an item is selected"""
    self.parent.combat.duels[4].unit.setfather(self.father_drop.selected_value)
    self.hp.text = self.parent.combat.duels[4].unit.maxhp
    self.strength.text = self.parent.combat.duels[4].unit.strength
    self.magic.text = self.parent.combat.duels[4].unit.magic
    self.skill.text = self.parent.combat.duels[4].unit.skill
