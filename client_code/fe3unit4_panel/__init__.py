...
class fe3unit4_panel(fe3unit4_panelTemplate):
  ...
  def unit_drop_change(self, **event_args):
    ...
    if self.parent.combat.duels[3].unit.name in ("Astram", "Abel", "Est", "Ogma", "Catria", "Gordin", "Samson", "Caeda", "Sheena", "Julian", "Sirius", "Cecil", "Palla", "Bantu", "Phina", "Merric", "Marth", "Midia", "Minerva", "Jubelo", "Yuliya", "Ryan", "Linde", "Luke", "Lena", "Roderick"):
  ...
  def support1_change(self, **event_args):
    ...
    self.parent.combat.duels[3].unit.setsupportbonus(self.support1.text, self.support1.checked)
    self.parent.combat.duels[3].unitdisplay()
    self.hit.text = self.parent.combat.duels[3].unit.hit
    self.crit.text = self.parent.combat.duels[3].unit.crit
  ...
  def support2_change(self, **event_args):
    """This method is called when this checkbox is checked or unchecked"""
    self.parent.combat.duels[3].unit.setsupportbonus(self.support2.text, self.support2.checked)
    self.parent.combat.duels[3].unitdisplay()
    self.hit.text = self.parent.combat.duels[3].unit.hit
    self.crit.text = self.parent.combat.duels[3].unit.crit
